...
def welcome(message):
   ...
   # keyboard simple(над клавой,и под полем ввода)
   markup = telebot.types.ReplyKeyboardMarkup(True, True)#тут удаляется кнопка
   item1 = types.KeyboardButton("😊Зарегистрироваться!")
   markup.add(item1)

   bot.send_message(message.chat.id, "Добро пожаловать, {0.first_name}!\nЯ - <b>{1.first_name}</b>, бот созданный чтобы помочь вам. Перед тем как начать, пожалуйста зарегистрируйтесь 😊.".format(message.from_user, bot.get_me()), parse_mode='html', reply_markup=markup)
...
def reg_fio(message):
   global fio
   fio = message.text
   bot.send_message(message.from_user.id, "Поздравляю. Вы закончили регистрацию.\n Внимательно просмотрите собой заполн инфор, если есть ошибки, нажмите кнопку редактировать" )
